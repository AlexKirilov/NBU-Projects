...
//Alex - Working hours
$open_hour_working_days;
$close_hour_working_days;
$open_hours_weekend_days;
$close_hourse_weekend_days;
...
function nlp_reservations_form() {
    ...
    $reservation_form .= "<div id='loading'></div>";
    ...
    $reservation_form .= "<div class='nlp_file_uploader'>";
//    $reservation_form .= '<form action="upload.php" method="post" enctype="multipart/form-data">
...
//    }
    ...
    if (isset($_POST['nlp_value_of_conf_email'], $_POST['nlp_value_of_hours'], $_POST['nlp_value_integer_month'], $_POST['nlp_value_integer_day'], $_POST['nlp_value_integer_year'], $_POST['value_of_chosen_hall'], $_POST['nlp_value_of_name']) && !empty($_POST['nlp_value_of_hours']) && !empty($_POST['nlp_value_of_conf_email']) && !empty($_POST['nlp_value_integer_month']) && !empty($_POST['nlp_value_integer_day']) && !empty($_POST['nlp_value_integer_year']) && !empty($_POST['value_of_chosen_hall']) && !empty($_POST['nlp_value_of_name'])) {
        ...
        $repeat_option_selected = filter_input(INPUT_POST, 'nlp_value_of_repeat', FILTER_SANITIZE_NUMBER_INT);
        $event_info = filter_input(INPUT_POST, 'nlp_value_of_info', FILTER_SANITIZE_STRING);
        if (isset($_FILES['file_attached']) && !empty($_FILES['file_attached'])) {
            die(var_dump($_FILES['file_attached']));
//            if (!empty($_POST['file_attached']['name'])) {
//                
//                // Setup the array of supported file types. In this case, it's just PDF.
//                $supported_types = array('application/pdf');
//
//                // Get the file type of the upload
//                $arr_file_type = wp_check_filetype(basename($_FILES['nlp_uploader_meta']['name']));
...
//                    $upload = wp_upload_bits($_FILES['nlp_uploader_meta']['name'], null, file_get_contents($_FILES['nlp_uploader_meta']['tmp_name']));
//
//                    if (isset($upload['error']) && $upload['error'] != 0) {
//                        wp_die('Имаше грешка при качването на файлът.Грешката е : ' . $upload['error']);
//                    } else {
//                        add_post_meta($id, 'nlp_uploader_meta', $upload);
//                        update_post_meta($id, 'nlp_uploader_meta', $upload);
//                    } // end if/else
//                } else {
//                    wp_die("Файлът който се опитвате да качите не е PDF.");
//                } // end if/else
//            } // end if
        }
        /* 18.08.2015 */
        if (isset($_POST['value_use_of_tech']) && !empty($_POST['value_use_of_tech'])) {
            $use_tech = filter_input(INPUT_POST, 'value_use_of_tech', FILTER_SANITIZE_NUMBER_INT);
        }
        /* 18.08.2015 */
        $nlp_value_send_by_ajax = array();
//         var_dump($_POST['nlp_value_of_hours']);
        ...
        if ($repeat_option_selected == 1) {
            ...
            wp_set_post_categories($post_id, $post_categories, false);
            update_post_meta($post_id, 'nlp_html_for_meta_box_end_time', $nlp_value_send_by_ajax);
            nlp_mail_sender_function($confirmation_email);
            update_post_meta($post_id, 'email_meta_box', $confirmation_email); /* 17/08/2015 */
            if ($use_tech != 999) { /* 18.08.2015 */
                update_post_meta($post_id, 'tehnika_meta_box', 1);  /* 18.08.2015 */
            } /* 18.08.2015 */
            echo "<div>Запазването на " . $date_current_year . "-" . $date_current_month . "-" . "$date_current_day" . " беше успешно.</div>";
        } else if ($repeat_option_selected == 4) {
            if (isset($_POST['value_of_repeating_months'], $_POST['value_number_of_months_to_repeat']) && !empty($_POST['value_number_of_months_to_repeat']) && !empty($_POST['value_of_repeating_months'])) {
...
//              var_dump($dates_possible_and_not);
            foreach ($dates_possible_and_not['da'] as $key => $val) {
                ...
                wp_set_post_categories($post_id, $post_categories, false);
                update_post_meta($post_id, 'nlp_html_for_meta_box_end_time', $nlp_value_send_by_ajax);
                update_post_meta($post_id, 'email_meta_box', $confirmation_email);
                if ($use_tech != 999) { /* 18.08.2015 */
                    update_post_meta($post_id, 'tehnika_meta_box', 1);  /* 18.08.2015 */
                } /* 18.08.2015 */
            }
...
//        $category_term_id = filter_input(INPUT_POST, 'nlp_value_integer_send', FILTER_SANITIZE_NUMBER_INT);
    ...
    $return_possible_days_months .= "<div class='nlp_more_information_for_event'><label for='nlp_event_information'>Повече информация за Събитието <br /> Моля въведете и други участници</label><textarea  name='nlp_event_information' class='nlp_event_information' name='ta' rows='4' cols='50' maxlength='250'></textarea><div id='textarea_feedback'></div></div>";

    /* 18.08.2015 */
    $return_possible_days_months .= "<div class='use_tech'>";
    $return_possible_days_months .= "<p>Ще използвате ли мултимедиен прежектор и екран ? </p>";
    $return_possible_days_months .= "<input type='checkbox' id='tehnika_meta_box' name='tehnika_meta_box' />Да";
    $return_possible_days_months .="</div>";
...
function nlp_day_ajax_reservate_posts() {
    ...
    if (isset($_POST['nlp_value_integer_month'], $_POST['value_of_chosen_hall'], $_POST['nlp_value_integer_year'], $_POST['nlp_value_integer_day']) && !empty($_POST['value_of_chosen_hall']) && !empty($_POST['nlp_value_integer_day']) && !empty($_POST['nlp_value_integer_month']) && !empty($_POST['nlp_value_integer_year'])) {
        ...
        $cash_variable = "";
        /* NOVO 17/08/2015 */
        $day_name_class = nlp_return_day_name_english($date_current_month, $date_current_year, $date_current_day);
        /* NOVO 17/08/2015 */
...
//            }
            /* NOVO 17/08/2015 */
            if ($day_name_class != 'saturday' && $day_name_class != 'sunday') {

                if ($value_block != "") {
                    $cash_variable .= "<div class='nlp_reserved_hours nlp_possible_reservation_hour'>Запазен " . nlp_human_translator_blocks($key_block, $day_name_class) . "</div>";
                } else {
                    $cash_variable .= "<div class='nlp_free_hours nlp_possible_reservation_hour'  id='nlp_target_" . $key_block . "'>Свободен " . nlp_human_translator_blocks($key_block, $day_name_class) . "<input type='checkbox' name='nlp_checked_block' class='nlp_checked_block' /></div>";
                }
            }
            /* NOVO 17/08/2015 */ else {

                if ($key_block != 0 && $key_block != 11 && $key_block != 12 && $key_block != 13) {
                    if ($value_block != "") {
                        $cash_variable .= "<div class='nlp_reserved_hours nlp_possible_reservation_hour'>Запазен " . nlp_human_translator_blocks($key_block, $day_name_class) . "</div>";
                    } else {
                        $cash_variable .= "<div class='nlp_free_hours nlp_possible_reservation_hour'  id='nlp_target_" . $key_block . "'>Свободен " . nlp_human_translator_blocks($key_block, $day_name_class) . "<input type='checkbox' name='nlp_checked_block' class='nlp_checked_block' /></div>";
...
function nlp_repeat_event_chosen() {
    if (isset($_POST['nlp_value_ichecked']) && !empty($_POST['nlp_value_ichecked'])) {
        ...
        if ($value_of_checkbox == 2) {
            ...
            $register_repeat .= "<p class='nlp_repeat_inforamtion'> ! Начало се смята текущо въведената дата и час по-горе във формата. !</p>";
 ...
 * Sort of objects by term_id 
 * 
 * 
 *  */

function sort_halls($a, $b) {
...
//            var_dump($category->name . "-" . $category->term_id);
            foreach ($category as $data_key => $data) {
    ...
    usort($cat_data, "sort_halls");
    foreach ($all_halls_id_names as $indexss => $valuess) {

        if ($cat_data[$indexss]->term_id != 1) {
            if (isset($cat_data[$indexss]) && !empty($cat_data[$indexss]) && $cat_data[$indexss]->name != 'default') {
                if ($cnt) {
                ...
                $return_days .= '<div class="cat_name" id="' . $cat_data[$indexss]->term_id . '">' . $cat_data[$indexss]->name . '</div>';
                $return_days .= '</div>';
            }
        }
    }
    ...
    $return_days .= '</div>';
    $return_days .= "<div id='loading'></div>";
    ...
    /* NOVO 17/08/2015 */
    $return_days .= "<div class='nlp_years_callendar'>";
    $return_days .= "<select class='year_calendar_view' name='selected_date' >";
    for ($i = (date('y') + 2000) - 5; $i <= (date('y') + 2000) + 5; $i++) {
        if ($i == (date('y') + 2000)) {
            $return_days .= "<option selected>" . $i . "</option>";
            $selected_date_optional = $i;
        } else {
            $return_days .= "<option>" . $i . "</option>";
        }
    }
//    var_dump($all_halls_id_names[0]);
    $return_days .= "</select>";
    $return_days .= "</div>";
    /* NOVO 17/08/2015 */
    ...
    $return_days .= "<div class='nlp_calendar_navigation'><div id='calendar_navigation_back' class='nlp_calendar_back'>Предишен</div><div class='curent_mnth_year'><div class='super_month'></div><div class='super_year'></div></div><div  id='calendar_navigation_next' class='nlp_calendar_next'>Следващ</div></div>";
    ...
    for ($k = 1; $k <= 12; $k++) {
        ...
        $return_days .= "<div id=" . $k . " class='nlp_months'>";
        for ($i = 1; $i <= $days_in_month; $i++) {
            ...
            if ($i == 1) {
                ...
                $return_days .= nlp_calendar_view($day_name_class) . "<div  id='" . $i . "' class='nlp_day_selected nlp_" . nlp_return_day_name_english($date_first_month, $date_current_year, $i) . "' value='" . $i . "' title='" . nlp_return_day_name($date_first_month, $date_current_year, $i) . "'> "  . $i . " / " . add_month_to_year_view($k) . "" . $inner_days_info .  "</div>";
            } else {
                $inner_days_info = "";
                ...
                if (!empty($taken_or_not)) {
                    foreach ($taken_or_not as $key => $index) {
                        ...
                        if ($index != "") {
                            ...
                            $inner_days_info .= "<div class='block_info'>" . nlp_human_translator_blocks($key) . " </div>";
                ...
                $return_days .= "<div  id='" . $i . "' class='nlp_day_selected nlp_" . nlp_return_day_name_english($date_first_month, $date_current_year, $i) . "' value='" . $i . "' title='" . nlp_return_day_name($date_first_month, $date_current_year, $i) . "'> "  . $i . " / " . add_month_to_year_view($k) . "" . $inner_days_info . "</div>";
            }
        }
        $return_days .="</div>";
    }
...
//        var_dump($hall_id);
        /* NOVO 17/08/2015 */
        $year_selected = filter_input(INPUT_POST, 'year_for_calendar', FILTER_SANITIZE_NUMBER_INT);
        /* NOVO 17/08/2015 */
        for ($k = 1; $k <= 12; $k++) {
            $date_first_month = $k;
//            $date_current_year = date('y');
            /* NOVO 17/08/2015 */
            $date_current_year = ($year_selected - 2000);
            /* NOVO 17/08/2015 */
            $days_in_month = cal_days_in_month(CAL_GREGORIAN, $date_first_month, $date_current_year);
            ...
            for ($i = 1; $i <= $days_in_month; $i++) {
                ...
                if ($i == 1) {
                    ...
                    $inner_days_info .="</div>";
//                    $return_days .= $hall_id . "cat number";
                    $return_days .= nlp_calendar_view($day_name_class) . "<div  id='" . $i . "' class='nlp_day_selected nlp_" . nlp_return_day_name_english($date_first_month, $date_current_year, $i) . "' value='" . $i . "' title='" . nlp_return_day_name($date_first_month, $date_current_year, $i) . "'> "  . $i . " / " . add_month_to_year_view($k) . "" . $inner_days_info . "</div>";
                } else {
                    $inner_days_info = "";
                    $inner_days_info .= "<div class='calendar_day_container'>";
                    $taken_or_not = nlp_check_if_time_is_taken(($date_current_year + 2000 ) . "-" . $date_first_month . "-" . $i, $hall_id);
                    $taken_or_not_info = nlp_check_if_time_is_taken_post(($date_current_year + 2000 ) . "-" . $date_first_month . "-" . $i, $hall_id);
 ...
 */
function nlp_add_meta_box_email() {
    ...
    add_meta_box(
            'email_meta_box'
            , 'Email'
            , 'nlp_add_meta_box_email_callback'
            , 'post'
            , 'advanced'
            , 'high'
    );
...
add_action('add_meta_boxes', 'nlp_add_meta_box_email');
 ...
 */
function nlp_add_meta_box_email_callback($post) {
...
//	wp_nonce_field( 'myplugin_save_meta_box_data', 'myplugin_meta_box_nonce' );
     ...
     */
    $value = get_post_meta($post->ID, 'email_meta_box', true);

    echo '<label for="email_meta_box">';

    echo '</label> ';
    echo '<input type="text" id="email_meta_box" name="email_meta_box" value="' . esc_attr($value) . '" size="25" />';
}
...
//	}
    ...
    if (!isset($_POST['email_meta_box'])) {
        return;
    }

    // Sanitize user input.
    $my_data = sanitize_text_field($_POST['email_meta_box']);

    // Update the meta field in the database.
    update_post_meta($post_id, 'email_meta_box', $my_data);
}
 ...
 * 
 * cqlata funkcionalnost za metabox uploader 
 * 
 *  */

function nlp_add_uploader_meta_boxes() {

    // Define the custom attachment for posts
    add_meta_box(
            'nlp_uploader_meta', 'Ъплоадер за Файлове', 'nlp_uploader_meta', 'post', 'side'
    );
}

// end add_custom_meta_boxes
add_action('add_meta_boxes', 'nlp_add_uploader_meta_boxes');

function nlp_uploader_meta() {

//    wp_nonce_field(plugin_basename(__FILE__), 'wp_custom_attachment_nonce');
    $html = '<p class="description">';
    $html .= 'Качете PDF файлът тук.';
    $html .= '</p>';
    $html .= '<input type="file" id="nlp_uploader_meta" name="nlp_uploader_meta" value="" size="25" />';
    $pdf_info = get_post_meta(get_the_ID(), 'nlp_uploader_meta', true);
//    get_template_part( 'content', 'single' );
    if (!empty($pdf_info)) {
        $html .= '<a href="' . $pdf_info['url'] . '">';
        $html .= "Изтеглете PDF Файлът от ТУК.";
        $html .= "</a>";
    } else {
        $html .= "<p> Няма качен файл. </p>";
    }
    echo $html;
}
...
//    } // end if
    ...
    if (!empty($_FILES['nlp_uploader_meta']['name'])) {
...
add_action('save_post', 'nlp_save_meta');
...
function nlp_claendar_year_view() {

    /* NOVO 17/08/2015 */
    if (isset($_POST['id_of_hall'], $_POST['year_for_calendar']) && !empty($_POST['id_of_hall']) && !empty($_POST['year_for_calendar'])) {
        /* NOVO 17/08/2015 */
        if (isset($_POST['user_id']) && !empty($_POST['user_id'])) {
            $usr_id = filter_input(INPUT_POST, 'user_id', FILTER_SANITIZE_NUMBER_INT);
        } else {
                ...
                if ($i == 1) {
                    ...
                    if (!empty($taken_or_not)) {
                        foreach ($taken_or_not as $key => $index) {
                            ...
                            if ($index != "") {
                                $get_data_post = get_post($taken_or_not_info[$key]);
                                $inner_days_info .= "<div class='block_info'>" . nlp_human_translator_blocks($key, $day_name_class) . " </div>";
                                $inner_days_info .= '<div class="hidden_info"><div class="hidden_title">' . $get_data_post->post_title . '</div><div class="hidden_description">' . $get_data_post->post_content . '</div>';
                                if ($usr_id != 0) {
                                    $inner_days_info .= "<div>" . get_post_meta($taken_or_not_info[$key], 'email_meta_box', true) . "</div>";

                                    $values = get_post_meta($taken_or_not_info[$key], 'tehnika_meta_box', true);
                                    if (empty($values)) {
                                        $value = 0;
                                    }
                                    $inner_days_info .=" <div> Използване на прожекторна техника и екран :";
                                    if ($value == 0) {
                                        $inner_days_info .= "Да";
                                    } else {
                                        $inner_days_info .= "Не";
...
//
//                            if ($index != "") {
//                                $inner_days_info .= "<div>" . nlp_human_translator_blocks($key, $day_name_class) . " zaet </div>";
//                            }
//                        }
//                    }
//                    $inner_days_info .= "TEST" . $i . ":" . $hall_id . " :" . $year_selected;
                    $inner_days_info .="</div>";

                    $return_days .= nlp_calendar_view($day_name_class) . "<div  id='" . $i . "' class='nlp_day_selected year_content_in_row nlp_" . nlp_return_day_name_english($date_first_month, $date_current_year, $i) . "' value='" . $i . "' title='" . nlp_return_day_name($date_first_month, $date_current_year, $i) . "'> " . $i . " / " . add_month_to_year_view($k) . "" . $inner_days_info . "</div>";
                } else {
                    $inner_days_info = "";
                    $inner_days_info .= "<div class='block_details'>";
                    ...
                    if (!empty($taken_or_not)) {
                        foreach ($taken_or_not as $key => $index) {
                            ...
                            if ($index != "") {
                                $get_data_post = get_post($taken_or_not_info[$key]);
                                $inner_days_info .= "<div class='block_info' id='".$get_data_post->ID."'>" . nlp_human_translator_blocks($key, $day_name_class) . " </div>";
                                $inner_days_info .= '<div class="hidden_info"><div class="hidden_title">' . $get_data_post->post_title . '</div><div class="hidden_description">' . $get_data_post->post_content . '</div>';
                                if ($usr_id != 0) {
                                    $inner_days_info .= "<div class='guest_info_mail'>" . get_post_meta($taken_or_not_info[$key], 'email_meta_box', true) . "</div>";
//                                    $inner_days_info .= "<div class='guest_info_mail'>E-mail: " . get_post_meta($taken_or_not_info[$key], 'email_meta_box', true) . "</div>";

                                    $values = get_post_meta($taken_or_not_info[$key], 'tehnika_meta_box', true);
                                    if (empty($values)) {
                                        $value = 0;
                                    }
//                                    $inner_days_info .=" <div  class='info_projector'> Използване на прожекторна техника и екран :";
                                    $inner_days_info .=" <div  class='info_projector'> Техника и екран : ";
                                    if ($value == 0) {
                                        $inner_days_info .= "Не";
                                    } else {
                                        $inner_days_info .= "Да";
                                    ...
                                    $inner_days_info .="</div>";
                                          ...
                                          $inner_days_info .= "  <a href='".get_edit_post_link( $get_data_post->ID )."' target='_blank' >Редактирай</a>";
                                   
                                  
                                    /* Alex - adding css for logged user */
                                    ?>
                                    <style>
                                        .year_content_in_row div .hidden_title{
                                            float: left;
                                            width: 30% !important;
                                        ...
                                        .year_content_in_row div .hidden_description{
                                            float: left;
                                            width: 30% !important;
                                        ...
                                        .year_content_in_row div .guest_info_mail{
                                            float: left;
                                            width: 21% !important;
                                        ...
                                        .year_content_in_row div .info_projector{
                                            float: left;
                                            width: 15% !important;
...
//                    $inner_days_info .= "Test" . $i . ":" . $hall_id . " :" . $year_selected;
                    ...
                    $return_days .= "<div  id='" . $i . "' class='nlp_day_selected year_content_in_row nlp_" . nlp_return_day_name_english($date_first_month, $date_current_year, $i) . "' value='" . $i . "' title='" . nlp_return_day_name($date_first_month, $date_current_year, $i) . "'> " . $i . " / " . add_month_to_year_view($k) . "" . $inner_days_info . "</div>";
                }
            }
            $return_days .="</div>";
        }
...
add_action('wp_ajax_nlp_claendar_year_view', 'nlp_claendar_year_view');
add_action('wp_ajax_nopriv_nlp_claendar_year_view', 'nlp_claendar_year_view');

function add_month_to_year_view($m_index) {
    $arr = [0, "Януари", "Февруари", "Март", "Април", "Май", "Юни", "Юли", "Август", "Септември", "Октомври", "Ноември", "Декември"];
    return $arr[$m_index];
}
 ...
 * Adds a box to the main column on the Post and Page edit screens.
 */
function nlp_add_meta_box_tehnika() {
    ...
    add_meta_box(
            'tehnika_meta_box'
            , 'Прожекторна техника със Екран'
            , 'nlp_add_meta_box_technika_callback'
            , 'post'
            , 'advanced'
            , 'high'
    );
...
add_action('add_meta_boxes', 'nlp_add_meta_box_tehnika');
 ...
 * @param WP_Post $post The object for the current post/page.
 */
function nlp_add_meta_box_technika_callback($post) {
...
//	wp_nonce_field( 'myplugin_save_meta_box_data', 'myplugin_meta_box_nonce' );
     ...
     * from the database and use the value for the form.
     */
    $value = get_post_meta($post->ID, 'tehnika_meta_box', true);
    if (empty($value)) {
        $value = 0;
    }
    echo '<label for="tehnika_meta_box">';

    echo '</label> ';
    if ($value == 0) {
        echo '<input type="radio" class="tehnika_meta_box" name="tehnika_meta_box"  value="1" size="25" />Да';

        echo '<input type="radio" class="tehnika_meta_box" name="tehnika_meta_box" value="0" checked size="25" />Не';
    } else {
        echo '<input type="radio" class="tehnika_meta_box" name="tehnika_meta_box" value="1" checked size="25" />Да';

        echo '<input type="radio" class="tehnika_meta_box" name="tehnika_meta_box"  value="0" size="25" />Не';
    }
}
 ...
 * @param int $post_id The ID of the post being saved.
 */
function tehnika_save_meta_box_data($post_id) {
...
//	}
    ...
    // Make sure that it is set.
    if (!isset($_POST['tehnika_meta_box'])) {
        return;
    }

    // Sanitize user input.

    $my_data = filter_input(INPUT_POST, 'tehnika_meta_box', FILTER_SANITIZE_NUMBER_INT);

    // Update the meta field in the database.
    update_post_meta($post_id, 'tehnika_meta_box', $my_data);
}

add_action('save_post', 'tehnika_save_meta_box_data');

/* 17/08/2015 */
/* 18/08/2015 */
/* Alex - 16/12/2015 Print button */

function div_print_button() {
    echo "<div id='div-print'><button type='button' id='btn-print' name='print' value='Print'></button></div>";
}

//add_action('print_page', 'div_print_button');
add_shortcode('print_page', 'div_print_button');
